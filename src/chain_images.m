function [PV] = chain_images(directory, matches, harris_on)
%CHAIN_IMAGES Summary of this function goes here
...
print_start("chaining images");

% Check the directory if PV was already computed
if harris_on == 1
    if exist(strcat(directory, 'PV_Harris.mat'))
        load(strcat(directory, 'PV_Harris.mat'));
        disp("PV_harris were loaded from PV_Harris.mat");
        print_end("chaining images with Harris");
        return
    end
else
    if exist(strcat(directory, 'PV.mat'))
        ...
        disp("PV was loaded from PV.mat");
        print_end("chaining images");
        return
    end
end
...
% Init amount of colums to #matches of first 
cols = size(matches{1},2);
PV = zeros(frames+1,cols);
...
disp(strcat(int2str(size(PV,2)), ' points in pointview matrix'));
toc;

% Save PV matrix
if harris_on == 1
    save(strcat(directory, 'PV_Harris.mat'), 'PV');
else
